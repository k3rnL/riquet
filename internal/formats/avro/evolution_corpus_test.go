	...
	for _, testCase := range cases {
		t.Run(testCase.Name, func(t *testing.T) {
			resolver := formats.ResolveFunc(func(_ context.Context, reference domain.Reference) (domain.Schema, error) {
				definition, ok := testCase.ReferenceSchemas[reference.Subject]
				if !ok {
					return domain.Schema{}, errors.New("reference not found")
				}
				return domain.Schema{Type: domain.SchemaTypeAvro, Definition: definition}, nil
			})
			parse := func(definition string) formats.Parsed {
				parsed, parseErr := engine.Parse(context.Background(), formats.ParseRequest{
					Definition: definition, References: testCase.References, Normalize: true,
				}, resolver)
				if parseErr != nil {
					t.Fatal(parseErr)
				}
